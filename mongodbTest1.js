...
var defaultDB = "GENDB";
var defaultCollection = "G_EMP";
var defaultURL = 'mongodb://admin:Root1234@9.123.197.164:27017/admin';
...
function queryQuery(res, key, callback){
	if(key && callback){
		try{
			_mongo.connect(defaultURL, function(err,conn){
				...
				coll.find({$or:[{cnum:field},{empName:field},{empNotesID:field},{empMailID:field}]},{limit:10}).toArray(function(err,docs){
					JSONP(res, docs, callback);
					conn.close();
				});
			});
...
function list(res, url, db, collection, callback){
	if(url && db && collection && callback){
		try{
			_mongo.connect(url, function(err,conn){
				newConn = conn.db(db);
				...
				coll.find({},{limit:100}).toArray(function(err,docs){
					var map = function() { for( var key in this){emit(this.user_id, 1);} };
					var reduce = function(k,vals) { return null; };
					docs.mapReduce(map, reduce, {out: {inline : 1}}, function(err, outCollection){
...
_http.createServer(function(req,res){
	...
	} else if (params.pathname == '/mongodb/query'){
		queryQuery(res, query.key, query.callback);
	} else{
